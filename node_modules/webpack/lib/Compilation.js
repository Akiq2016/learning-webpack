...
class Compilation extends Tapable {
	...
	processModuleDependencies(module, callback) {
		...
		for (const pair1 of dependencies) {
			console.log('dependencies:', pair1);
	...
	_addModuleChain(context, dependency, onModule, callback) {
		...
		console.log('获取对应的工厂方法 moduleFactory:', moduleFactory.constructor)
		if (!moduleFactory) {
			throw new Error(
		...
		this.semaphore.acquire(() => {
			console.log('调用moduleFactory.create')
			moduleFactory.create(
				...
				(err, module) => {
					...
					console.log('创建好了module：', module.constructor);
					...
					const afterBuild = () => {
						if (addModuleResult.dependencies) {
							console.log('存在addModuleResult.dependencies，', addModuleResult.dependencies);
					...
					if (addModuleResult.build) {
						console.log('build module:', "this.buildModule(module)")
	...
	addEntry(context, entry, name, callback) {
		...
		console.log('addEntry 为了调用 _addModuleChain', context, entry);
	 ...
	 */
	seal(callback) {
		console.log(`在所有模块及其依赖模块 build 完成后，webpack 会监听 seal 事件调用各插件对构建后的结果进行封装，
		要逐次对每个 module 和 chunk 进行整理，生成编译后的源码，合并，拆分，生成 hash 。 
		同时这是我们在开发时进行代码优化和功能添加的关键环节。`)
		console.log(`seal 方法包含了优化、分块、哈希，编译停止接收新模块，开始生成 chunks。
		此阶段依赖了一些 webpack 内部插件对 module 进行优化，为本次构建生成的 chunk 加入 hash 等。`)
		...
		console.log("_preparedEntrypoints 在 addEntry 方法中被填充，它存放着 entry 名称和对应的 entry module:: this._preparedEntrypoints: ", this._preparedEntrypoints)
		console.log("// 将 entry 中对应的 module 都生成一个新的 chunk")
		for (const preparedEntrypoint of this._preparedEntrypoints) {
			const module = preparedEntrypoint.module;
			const name = preparedEntrypoint.name;
			console.log("创建chunk: this.addChunk(name);", name);
			const chunk = this.addChunk(name);
			console.log("创建entrypoint(继承自ChunkGroup): new Entrypoint(name)", name);
		...
		console.log('todo遍历 module 的依赖列表，将依赖的 module 也加入到 chunk 中')
		buildChunkGraph(
			this,
		...
		console.log('优化: SplitChunksPlugin 会监听 optimizeChunksAdvanced，抽取公共模块，形成新的 chunk')
		console.log("优化：RuntimeChunkPlugin 会监听 optimizeChunksAdvanced，抽离 runtime chunk")
		while (
		...
		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, err => {
			...
			if (this.hooks.shouldGenerateChunkAssets.call() !== false) {
				console.log("this.hooks.beforeChunkAssets.call(); 通过这个钩子 可以在生成 assets 之前修改 chunks")
				this.hooks.beforeChunkAssets.call();
				console.log("通过chunks 生成最终assets 「createChunkAssets」--------------------------------")
	...
	createChunkAssets() {
		console.log('开始createChunkAssets', this.outputOptions)
		const outputOptions = this.outputOptions;
		const cachedSourceMap = new Map();
		/** @type {Map<string, {hash: string, source: Source, chunk: Chunk}>} */
		const alreadyWrittenFiles = new Map();
		console.log("this.chunks.length: ", this.chunks.length)
		for (let i = 0; i < this.chunks.length; i++) {
			...
			try {
				console.log('======')
				console.log('判断chunk要使用mainTemplate还是chunkTemplate')
				console.log("如果 chunk 包含 webpack runtime 代码，就用 mainTemplate 来渲染，否则用 chunkTemplate 来渲染")
				const template = chunk.hasRuntime()
					? this.mainTemplate
					: this.chunkTemplate;
